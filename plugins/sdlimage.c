...
int setTextUTF8(char *strTmp){
    ...
    slog(LVL_NOISY,DEBUG,"Set text to texture %s at (%d,%d) : %s",name, x, y, text);
...
//   slog(LVL_NOISY,DEBUG,"Set text at (%d,%d) rot %d : %s",x,y,rot,text);
...
int setAutoRender(char *t){
   if(strncmp(t,"true",4) == 0){
      ph->autorender = true;
      slog(LVL_NOISY,DEBUG,"autorender is now on");
      return 0;
   }
   else if(strncmp(t,"false",5) == 0)
   {
      slog(LVL_NOISY,DEBUG,"autorender is now off");
      ph->autorender = false;
      return 0;
   }
...
bool setTextEx(char *name, int x, int y,int rotation, const char *text, char *fontName, char *colorName, int textType)
{
   ...
   slog(LVL_NOISY,DEBUG,"Rendering text into surface with rot %d",rotation);
   switch(textType){
      case TEXT_UTF8:
   ...
   slog(LVL_NOISY,DEBUG,"setTexture(%s,{%d,%d,%d,%d)",name,x,y,dest.w,dest.h);
...
int createColor(char *strTmp){
   ...
   if(!name || !getNumHex(cS,&col) || !getNumHex(aS,&alpha)){
      slog(LVL_NOISY,DEBUG,"Wrong parameter createColor(name FFFFFF FF) : %s",str);
      return false;
   }
   Color *c=malloc(sizeof(Color *));
   hexToColor(col,c);
   c->a = alpha;
   slog(LVL_NOISY,DEBUG,"Creating new color %s={%d,%d,%d} alpha = %d",name,c->r,c->g,c->b,c->a);
...
int clearTexture(char *str){
   slog(LVL_NOISY,DEBUG,"Clearing texture %s.",str);
...
int waitForTexture(char *str){
   ...
   while( timeout > 0 ){
      ...
      if(t) break;
      slog(LVL_NOISY,DEBUG,"Waiting for texture %s another %d seconds",name,timeout);
      timeout--;
      sleep(1);
   }
...
int setImageScaleSW(char *str)
{
   ...
   slog(LVL_NOISY,DEBUG,"Rescaling surface to Rect({%d,%d,%d,%d}), lock : %d )",rect.x,rect.y,rect.w,rect.h,src->locked);
...
int setImageScale(char *str)
{
   ...
   rect.x = 0;
   rect.y = 0;
   slog(LVL_NOISY,DEBUG,"Calling scaleToTexture(%s,Rect({%d,%d,%d,%d}) )",TI->name,rect.x,rect.y,rect.w,rect.h);
   scaleToTexture(TI,text,&rect);
   renderActive(TI->name);
   SDL_DestroyTexture(text);
   ph->textureCount--;
...
int setImage(char *name,int x, int y,const char *fileName){
   ...
   rect.x = x;
   rect.y = y;
   slog(LVL_NOISY,DEBUG,"Texture(%d,%d)",rect.w,rect.h);
...
char *initPlugin(pluginHandler *_ph){
    ...
    slog(LVL_NOISY,FULLDEBUG,"Plugin SDL2 initialized. PH is at 0x%x",ph);
    return PLUGIN_SCOPE;
}
