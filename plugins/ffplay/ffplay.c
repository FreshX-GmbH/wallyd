...
typedef void (*dcb_func)(VideoState *is, void *);
...
void setDisplayTextureCallback(void *f){
    displayTex=f;
}
...
//        }
        ...
        if (!is->texture) {
            if(createTex){
                is->texture = createTex(is);
            } else {
                is->texture = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_YV12, SDL_TEXTUREACCESS_STREAMING, vp->width, vp->height);
            }
        }
        ...
        if (SDL_LockTexture(is->texture, NULL, &pixels, &pitch) < 0) {
        ...
        SDL_UnlockTexture(is->texture);
        // Call our external callback
        if(displayTex){
            displayTex(is,is->texture);
        } else {
            SDL_UnlockTexture(is->texture);
...
#ifdef WALLY_PLUGIN
extern int video_open(VideoState *, int);
#else
...
//    }
    ...
    vp->picture = (AVPicture*)av_frame_alloc();

    vp->width = is->video_st->codec->width;
    vp->height = is->video_st->codec->height;
    vp->numBytes = avpicture_get_size(AV_PIX_FMT_YUV420P, vp->width, vp->height);
    vp->buffer = av_malloc(vp->numBytes * sizeof(uint8_t));
    
    avpicture_fill((AVPicture*) vp->picture, vp->buffer, AV_PIX_FMT_YUV420P,vp->width, vp->height);
...
#else
        int ret;
        callWithData("ffvideo::alloc_picture",&ret,is);
#endif
...
#else
int ff_event_handler(VideoState *cur_stream,SDL_Event *event)
{
    double incr, pos, frac;
    switch (event->type) {
        case FF_ALLOC_EVENT:
...
int Xffplay(int argc, char **argv)
{
    int flags;
    VideoState *is;
    ...
    show_banner(argc, argv[1], options);

    parse_options(NULL, argc, argv[1], options, opt_input_file);
...
//    SDL_EventState(SDL_ACTIVEEVENT, SDL_IGNORE);
    ...
    input_filename = "/Users/sunjun/Desktop/desktop/test.avi";
...
VideoState *renderFFVideo(char *filename)
{
    int flags;
    char dummy_videodriver[] = "SDL_VIDEODRIVER=dummy";
