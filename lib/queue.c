/*
 * https://github.com/vishalkanaujia/priqueue
 *
...
void pq_add_a_node(ptable* p, node** last, node** m, int key, int priority)
{
  ASSERT(p);
 ...
 */
void pq_put_buf(ptable* p, void* buf)
{
  if (p->buf_pool) {
    node* head = (node*)buf;
...
void pq_display_buf_pool(ptable* p)
{
  ASSERT(p);
...
void pq_create_pool(ptable** p, uint32_t num)
{
  node* head= NULL;
  node* temp = NULL;
...
void pq_create(ptable* p)
{
  ...
  pq_create_pool(&p, BUF_POOL_SIZE);
 ...
 */
void pq_put_data(ptable* p, int key, int priority)
{
  ASSERT(p);
  ASSERT(priority < PRI_MAX);

  pq_add_a_node(p, &(p->last[priority]), &(p->entry[priority].n),
                key, priority);
}
 ...
 */
void pq_get_data(ptable* p, int* key, int* pri)
{
  ASSERT(p);
